...
result = lineSearch(    @func_Rosenbrock,...       %Minimize func_example, defined in func_example.m (@ refers to a function).
                        @grad_Rosenbrock,...       %Corresponding gradient function, defined in grad_example.m.
                        @hes_Rosenbrock,...                  %Hessian needed for Newton
                        x0,...
                        [],...                  %Initial approximated hessian not needed.
                        iterationLimit,...
                        epsilon,...
                        @p_newton,...  %Directions by steepest descent method (note again the @, so we pass a function).
